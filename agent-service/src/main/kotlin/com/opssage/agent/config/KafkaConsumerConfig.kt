...
class KafkaConsumerConfig {
    ...
    fun kafkaErrorHandler(
        ...
        properties: AgentKafkaProperties,
    ): DefaultErrorHandler =
        DefaultErrorHandler(
            DeadLetterPublishingRecoverer(kafkaTemplate) { record, _ ->
                TopicPartition(
                    agentDlqTopic(record.topic(), properties),
                    record.partition(),
                )
            },
    ...
    private fun agentDlqTopic(
        topic: String,
        properties: AgentKafkaProperties,
    ): String =
        if (
            topic ==
            properties.topics.messages.investigationMessagesTopic
        ) {
            properties.dlq.investigationMessagesTopic
        } else {
            properties.dlq.investigationRequestsTopic
        }
}
